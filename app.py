...
def jobs():
    ...
    # Mark matched jobs and sort them first
    matched_jobs = []
    other_jobs = []
    for job in all_jobs:
...
def profile_management():
    ...
    user=users_col.find_one({'_id':ObjectId(session['user_id'])})
    results=list(results_col.find({'user_id':ObjectId(session['user_id'])}))
    # Fetch uploaded resume info from GridFS if available
    resume_file = None
    ...
    return render_template('student/profile_management.html',
        user=user,results=results,badges=user.get('badges',[]),resume_file=resume_file)
...
def student_profile():
    ...
    if user.get('uploaded_resume_id'):
        try:
            ...
            pass
    return render_template('student/student_profile_page.html', user=user, results=results, resume_file=resume_file)
